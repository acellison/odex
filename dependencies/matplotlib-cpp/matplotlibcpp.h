...
#  endif
    ...
    _interpreter() {
        ...
        if (!pyplotname || !matplotlibname || !mpl_toolkits ||
            !axis3d) {
...
PyObject* get_array(const Container& v)
{
    detail::_interpreter::get();    //interpreter needs to be initialized for the numpy commands to work

    npy_intp dims[1]; dims[0] = v.size();
    PyObject* varray = PyArray_ZEROS(1, dims, NPY_DOUBLE, false);
    for (std::size_t ii = 0; ii < v.size(); ++ii)
    {
        double* p = (double*)PyArray_GETPTR1((PyArrayObject*)varray, ii);
        *p = v[ii];
    }
    return varray;
}
...
template<typename Container>
PyObject* get_matrix(const Container& v)
{
    detail::_interpreter::get();    //interpreter needs to be initialized for the numpy commands to work

    npy_intp dims[2]; dims[0] = v.size(); dims[1] = v[0].size();
    PyObject* varray = PyArray_ZEROS(2, dims, NPY_DOUBLE, false);
    for (std::size_t ii = 0; ii < v.size(); ++ii)
    {
        for (std::size_t jj = 0; jj < v[ii].size(); ++jj)
        {
            double* p = (double*)PyArray_GETPTR2((PyArrayObject*)varray, ii, jj);
            *p = v[ii][jj];
        }
    }
    return varray;
...
using KeywordArgs = std::map<std::string, std::string>;
...
public:
    ...
    void set_object_ptr(PyObject* ptr)
    {
        if (m_ptr)
        {
            Py_DECREF(m_ptr);
    ...
    PyObject* object() { return m_ptr; }

private:
    PyObject* m_ptr;
};

Object dict_to_kwargs(KeywordArgs const& keywords)
{
    PyObject* kwargs = PyDict_New();
...
public:
    ...
    inline Axes gca(KeywordArgs const& kwargs={}) const;
};
...
public:
    ...
    Axes(Figure const& fig, KeywordArgs const& kwargs={})
    {
        *this = fig.gca(kwargs);
    }

    template <class XContainer, class YContainer>
    void plot(XContainer&& x, YContainer&& y, KeywordArgs const& keywords={}) const
    {
        ...
        PyObject* kwargs = dict_to_kwargs(keywords);
    ...
    void plot(XContainer&& x, YContainer&& y, ZContainer&& z, KeywordArgs const& keywords={}) const
    {
        assert(x.size() == y.size());
        ...
        Object kwargs = dict_to_kwargs(keywords);

        PyObject *plotfn = PyObject_GetAttrString(object(), "plot");
        if (!plotfn) throw std::runtime_error("No plot!");

        PyObject *res = PyObject_Call(plotfn, args, kwargs);
    ...
    template <class XContainer, class YContainer, class ZContainer>
    void plot_surface(XContainer&& x, YContainer&& y, ZContainer&& z, KeywordArgs const& keywords={}) const
    {
        assert(x.size() == y.size());
        assert(x.size() == z.size());

        // using numpy arrays
        PyObject* xarray = get_matrix(x);
        PyObject* yarray = get_matrix(y);
        PyObject* zarray = get_matrix(z);
        ...
        PyObject* kwargs = dict_to_kwargs(keywords);
         ...
         PyObject *plotfn = PyObject_GetAttrString(object(), "plot_surface");
        if (!plotfn) throw std::runtime_error("No surface");
        PyObject *res = PyObject_Call(plotfn, args, kwargs);
    ...
    void view_init(double elevation, double azimuth) const
    {
        // Set the elevation and azimuth
        PyObject *view = PyObject_GetAttrString(object(), "view_init");

        PyObject *viewargs = PyTuple_New(2);
        PyTuple_SetItem(viewargs, 0, PyFloat_FromDouble(elevation));
        PyTuple_SetItem(viewargs, 1, PyFloat_FromDouble(azimuth));
        PyObject_Call(view, viewargs, nullptr);
...
Axes Figure::gca(KeywordArgs const& kwargs) const
{
    Object gca = PyObject_GetAttrString(object(), "gca");
    if (!gca) throw std::runtime_error("No gca");

    Object gca_kwargs = dict_to_kwargs(kwargs);
    PyObject* ax = PyObject_Call(gca, detail::_interpreter::get().s_python_empty_tuple, gca_kwargs);

    return Axes(ax);
}

inline std::tuple<Figure, Axes> subplots(KeywordArgs const& kwargs={})
{
    Figure fig;
    Axes ax(fig, kwargs);
    return { std::move(fig), std::move(ax) };
}

template<typename Container>
bool plot(const Container &x, const Container &y, const KeywordArgs& keywords={})
{
    auto [fig, ax] = subplots();
    ax.plot(x, y, keywords);

    return true;
}

template<typename Container>
void plot(const Container &x, const Container &y, const Container &z, const KeywordArgs& keywords={})
{
    auto [fig, ax] = subplots({{"projection", "3d"}});
    ax.plot(x, y, z, keywords);
}

template<typename Container>
void plot_surface(const Container &x, const Container &y, const Container &z,
                  const KeywordArgs& keywords={})
{
    auto [fig, ax] = subplots({{"projection", "3d"}});
    ax.plot_surface(x, y, z, keywords);
}

template<typename Container>
void plot_surface(const Container &z, const KeywordArgs& keywords={})
{
    std::size_t rows = z.size();
    std::size_t cols = z[0].size();
    ...
    {
        std::iota(x[row].begin(), x[row].end(), 0.0);
        std::fill(y[row].begin(), y[row].end(), row);
    }
    plot_surface(x,y,z,keywords);
}

template<typename Container>
void plot_waterfall(const Container &x, const Container &y, const Container &z,
                    const KeywordArgs& keywords={})
{
    std::map<std::string, std::string> gca_kwargs = { {"projection", "3d"} };
    auto [fig, axis] = subplots(gca_kwargs);

    for (std::size_t ii = 0; ii < z.size(); ++ii)
    {
        axis.plot(x[ii], y[ii], z[ii], keywords);
    }
...
template<typename Container>
void plot_waterfall(const Container &z, const KeywordArgs& keywords={})
{
    std::size_t rows = z.size();
    std::size_t cols = z[0].size();
    std::vector<std::vector<double>> x(rows, std::vector<double>(cols, 0.0));
...
inline void gca()
{
    PyObject* res = PyObject_CallObject(detail::_interpreter::get().s_python_function_gca, detail::_interpreter::get().s_python_empty_tuple);
...
inline void zlabel(const std::string &str)
{
    ...
    PyObject *zlabelfn = PyObject_GetAttrString(axis, "set_zlabel");
    if (!zlabelfn) throw std::runtime_error("No plot!");
    Py_INCREF(zlabelfn);

    PyObject *res = PyObject_Call(zlabelfn, args, nullptr);
    if (!res) throw std::runtime_error("failed zlabel");
...
inline bool plot(const std::vector<double>& x, const std::vector<double>& y, const std::map<std::string, std::string>& keywords) {
    return plot<std::vector<double>>(x,y,keywords);
}
